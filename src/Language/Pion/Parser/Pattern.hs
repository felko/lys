...
split = keyword Token.Split >> Split <$> conjunction Token.Brace pattern'

select :: Parser Pattern
select = keyword Token.Select >> Select <$> located label <*> pattern'

extract :: Parser Pattern
extract = keyword Token.Extract >> Extract <$> pattern'

unwrap :: Parser Pattern
unwrap = keyword Token.Unwrap >> Unwrap <$> pattern'

copy :: Parser Pattern
copy =
  keyword Token.Copy >> Copy
    <$> between Token.Brace (pattern' `sepBy` Token.Comma)
